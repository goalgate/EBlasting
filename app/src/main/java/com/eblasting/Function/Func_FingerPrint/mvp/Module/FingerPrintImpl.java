package com.eblasting.Function.Func_FingerPrint.mvp.Module;
...
import com.eblasting.Function.Func_FingerPrint.mvp.powerControl.PowerControl;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (w_nEnrollStep < w_nGenCount) {
                    m_strPost = String.format("ID: %d,请放置手指 #%d!",m_nUserID, w_nEnrollStep + 1);
                ...
                if (w_nRet != DevComm1.ERR_SUCCESS) {
                    if (w_nRet == DevComm1.ERR_DUPLICATION_ID)
                        m_strPost = String.format("登记失败，已有相同模板ID = %d", w_nDupID[0]);
                    else
                        m_strPost = GetErrorMsg(w_nRet);
                } else {
                    m_strPost = String.format("模板 No : %d 登记成功, ", m_nUserID);
